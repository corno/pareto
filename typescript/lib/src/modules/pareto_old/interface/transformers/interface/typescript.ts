...
export const temp_create_file_path = (
    ...
    const valid_file_name = ($: string): string => {
        return ser_file_name.File_Name($)
    }
...
> = (
    ...
    p_.from.dictionary($).map(
        ($): s_out.Node => p_.from.state($).decide(
            ($): s_out.Node => {
                switch ($[0]) {
                    case 'package': return p_.option($, ($) => sh.n.file(
                        sh.source_file(
            ...
            switch ($[0]) {
                ...
                case 'reference': return p_.option($, ($) => p_.literal.list([
                    sh.s.type_alias(
                        ...
                        p_.literal.list([]),
                        p_variables(() => {
                            const Reference = ($: s_in_interface.Value.reference) => sh.t.type_reference(
                                //start
                                p_.from.state($.location).decide(
                                    ($) => {
                                        switch ($[0]) {
