...
export const deprecated_get_base_name = (path: string): _et.Optional_Value<string> => {
    const temp = op_split(path, { separator: 47 }) // '/'

    return op_pop_last_element(temp).map(($) => $.element)
}

export const deprecated_get_directory_path = (path: string): _et.Optional_Value<_et.List<string>> => {
    const temp = op_split(path, { separator: 47 }) // '/'
